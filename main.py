...
async def work_weather(message: types.message):
    ...
    weather_answer = (
        """<b>В Москве сейчас {}℃ (mаx={}℃ min={}℃)</b>""".format(weather_temp, weather_temp_max, weather_temp_min))
...
async def send_admin_startup(dp):
    await bot.send_message(chat_id=const.admin_id, text="<b>Бот запущен</b>")
